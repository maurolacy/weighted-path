// Safe Rust Fibonacci Heap implementation using Rc<RefCell> for comparison
// This version avoids raw pointers but may have different performance characteristics
...
pub struct SafeNode {
    ...
    child: Option<Rc<RefCell<SafeNode>>>,
    left: Option<Rc<RefCell<SafeNode>>>,
    right: Option<Rc<RefCell<SafeNode>>>,
}
...
pub struct SafeFibonacciHeap {
    min: Option<Rc<RefCell<SafeNode>>>,
}

impl SafeFibonacciHeap {
    pub fn new() -> Self {
        SafeFibonacciHeap { min: None }
    }

    pub fn insert(&mut self, key: u32, node_id: usize) -> Rc<RefCell<SafeNode>> {
        let node = Rc::new(RefCell::new(SafeNode::new(key, node_id)));
    ...
    fn add_to_root_list(&mut self, node: &Rc<RefCell<SafeNode>>) {
    ...
    fn consolidate(&mut self) {
        let max_degree = 50;
        let mut degree_table: Vec<Option<Rc<RefCell<SafeNode>>>> = vec![None; max_degree];
    ...
    fn link(&mut self, y: &Rc<RefCell<SafeNode>>, x: &Rc<RefCell<SafeNode>>) {
    ...
    fn cascading_cut(&mut self, node: &Rc<RefCell<SafeNode>>) {
...
impl Default for SafeFibonacciHeap {
    fn default() -> Self {
        Self::new()
    }
}
